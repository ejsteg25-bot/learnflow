 ...
 * VERSION: SOURCE BLOCK PIPELINE v0.5.7-BETA-PROMPT-MERGE
 *
 * STATUS:
 * - UNVERIFIED — NOT CONFIRMED STABLE.
 *
 * PURPOSE:
 * - Preserve verified v0.5.5 parser behavior.
 * - Restore DOCX upload, topic input, diagnostics, and dashboard.
 * - Add hardened Phase 3 mirror prompt.
 * - Clear mirror preview when switching questions.
 */

const { useState, useEffect } = React;

const APP_VERSION = "VERSION: SOURCE BLOCK PIPELINE v0.5.7-BETA-PROMPT-MERGE";
...
function App() {
  ...
  function detectEndLineAnswer(rawText) {
    ...
    return match[1].trim().length >= 2
      ? { stem: match[1].trim(), answer: match[2].toUpperCase() }
      : null;
  }
  ...
  function detectType(rawText, choiceCount) {
    ...
    if (/drag|move|blank|_{3,}|☐|token bank|drop zone/i.test(lower)) {
      return "INTERACTIVE";
    }
    ...
    if (detectEndLineAnswer(rawText) && choiceCount === 0) {
      return "MATCHING";
    }
  ...
  function harvest() {
    try {
      ...
      const processed = groups.map(lines => {
        ...
        const sourceNumber =
          String(lines[0] || "").match(/^\s*(\d{1,3})[.)]/)?.[1] || "?";
      ...
      setMode("Dashboard");
      setDiagnostics(
        `Raw: ${allLines.length} | Assess: ${assessmentLines.length} | Blocks: ${processed.length} | Grid Stopped: ${stoppedAtGrid ? "yes" : "no"}`
      );
    } catch (err) {
...
6. If the item requires a graph, image, diagram, table, segment, chart, or figure, do not invent the visual. Return NEEDS_TEACHER_REVIEW unless enough source context is present.
7. If answer choices exist, create parallel answer choices and identify the correct answer.
8. If source choices are merged together, separate them cleanly before mirroring.
9. If the item is interactive or fill-in-the-blank, preserve that format.
10. If the item is multi-select, preserve multi-select behavior.
11. Do not include scantron instructions, formatting artifacts, or document noise.
    ...
    mode === "Review" && active && React.createElement("div", null,
      ...
      React.createElement("div", {
        ...
        React.createElement("div", null,
          ...
          React.createElement("button", {
            onClick: () => setMirrorOutput(generateMirrorPrompt(active)),
            style: {
              marginBottom: "10px",
              padding: "10px 20px",
              background: "#28a745",
